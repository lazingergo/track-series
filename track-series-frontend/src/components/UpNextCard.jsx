...
export default function UpNextCard({ item, onMarkWatched, onOpenSeries }) {
  ...
  return (
    <div className="flex flex-row items-center bg-tvcard rounded-xl overflow-hidden shadow-lg border border-gray-800 transition-transform hover:scale-[1.01]">
      <button
        type="button"
        onClick={() => onOpenSeries?.(item.seriesId)}
        className="flex flex-row items-center flex-grow text-left"
      >
        <img
